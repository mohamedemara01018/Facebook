...
function Head_middle() {
    ...
    return (
        <ul className="midhead-ul-container">
            {
                middleData.map((item) => {
                    return <Link to={item.data} className="li"  data-click={click == item.id ? 'true' : 'false'} key={item.id} onClick={() => setClick(item.id)} onMouseEnter={() => setHover(item.id)} onMouseLeave={() => setHover(0)} >
                        <a href="">
