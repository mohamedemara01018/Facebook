    ...
    <div className="home-middle-container">
      {
        !loading && posts && posts.data ? posts['data'].map((item) => {
          return <div key={item.id} className="home-middle-post">
            ...
            <div className="post-para" onClick={() => { console.log(item.id) }}>
              <p>{item.body}</p>
            </div>
            <div className="post-img">
              <Link to={`post/${item.id}`}><img src={item.image} alt="" /></Link>
            </div>
            <Link to={`post/${item.id}`}><h1 dir="rtl">{item.comments_count} تعليق</h1></Link>
            <hr />
